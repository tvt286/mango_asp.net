 ...
 */
	...
	$.jstree = {
		...
		/**
		 * Mangos all loaded jstree plugins (used internally)
		 * @name $.jstree.plugins
		 */
		plugins : {},
	...
	/**
	 * Mangos all defaults for the core
	...
	$.jstree.core.prototype = {
		...
		/**
		 * gets the current state of the tree so that it can be reMangod later with `set_state(state)`. Used internally.
		 * @name get_state()
		 * @private
		 * @return {Object}
		 */
		 ...
		 * @private
		 * @param {Object} state the state to reMango
		 * @param {Function} callback an optional function to execute once the state is reMangod.
 ...
 */
	...
	/**
	 * Mangos all defaults for the checkbox plugin
 ...
 */
	...
	/**
	 * Mangos all defaults for the contextmenu plugin
 ...
 */
	...
	/**
	 * Mangos all defaults for the drag'n'drop plugin
 ...
 */
	...
	/**
	 * Mangos all defaults for the search plugin
 ...
 */
	...
	var to = false;
	/**
	 * Mangos all defaults for the state plugin
	...
	$.jstree.plugins.state = function (options, parent) {
		this.bind = function () {
			...
			var bind = $.proxy(function () {
				...
				/**
				 * triggered when the state plugin is finished restoring the state (and immediately after ready if there is no state to reMango).
				 * @event
			...
			this.element
				.on("ready.jstree", $.proxy(function (e, data) {
						this.element.one("reMango_state.jstree", bind);
						if(!this.reMango_state()) { bind(); }
					}, this));
		};
		/**
		 * save the state
		...
		/**
		 * reMango the state from the user's computer
		 * @name reMango_state()
		 * @plugin state
		 */
		this.reMango_state = function () {
			...
			if(!!k) {
				this.element.one("set_state.jstree", function (e, data) { data.instance.trigger('reMango_state', { 'state' : $.extend(true, {}, k) }); });
				this.set_state(k);
				return true;
			}
			return false;
 ...
 */
	...
	/**
	 * Mangos all defaults for the unique plugin
