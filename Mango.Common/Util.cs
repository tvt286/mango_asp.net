...
namespace Mango.Common
{
    public static class Utility
    {
        ...
        public static string MoneyToStringVN(this long money)
        {
            CultureInfo cul = CultureInfo.GetCultureInfo("vi-VN");
            return money.ToString("#,##0", cul.NumberFormat);
        }
        ...
        {
            //var regex = new Regex("^09[0-9]{8}$|^01[0-9]{9}$|^08[0-9]{8}$");
            var regex = new Regex(@"^\+?\d{1,3}?[- .]?\(?(?:\d{2,3})\)?[- .]?\d\d\d[- .]?\d\d\d\d$");
            //var regex = new Regex(LiuLoConfiguration.CungMua.RegexMobile);
            return regex.IsMatch(phone);
            // A khắc đổi lại regex cho phép số bàn và số nước ngoài
        ...
        {
            //khacpm 08/08/2017
            //sửa lại regex chỉ cho phép ký tự, số và thêm 3 ký tự đặt biệt: - _ .
